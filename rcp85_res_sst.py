...
def rcp85_res_sst ():
    ...
    mask_patches_lr = iceshelf_mask(elements_lr)
    elements_hr, patches_hr = make_patches(mesh_paths[1], circumpolar, mask_cavities)
    mask_patches_hr = iceshelf_mask(elements_hr)
    ...
    for expt_number in range(len(expt_names)):
        ...
        for panel in range(3):
            ...
            if expt_number == 0:
                ...
                patches = patches_lr
                mask_patches = mask_patches_lr
            elif expt_number == 1:
                elements = elements_hr
                patches = patches_hr
                mask_patches = mask_patches_hr
            for elm in elements:
