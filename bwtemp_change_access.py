...
def bwtemp_change_access():
    ...
    # Read temperature in control simulation (1992-2005 climatology)
    file = Dataset(control_file, 'r')
    # Annually average
    data1 = mean(file.variables['temp'][:,:], axis=0)
    file.close()
    # Same for RCP 4.5 A simulation (2006-2015)
    file = Dataset(access_file, 'r')
    data2 = mean(file.variables['temp'][:,:], axis=0)
    file.close()
    # Get difference
    data = data2 - data1
    values = []
    # Loop over elements
    for elm in elements:
        # For each element in an ice shelf cavity, find the bottom water
        # temperature and append the mean value for the 3 component Nodes
        if elm.cavity:
            values_tmp = []
    ...
    axis('off')
    title(r'Change in bottom water temperature in cavities ($^{\circ}$C)' + '\nRCP 4.5 A (2006-2015) vs control (1992-2005)', fontsize=30)
    cbar = colorbar(img, extend='both')
    cbar.ax.tick_params(labelsize=20)

    #fig.show()
    fig.savefig('bwtemp_change_access.png')


# Command-line interface
if __name__ == "__main__":

    bwtemp_change_access()
