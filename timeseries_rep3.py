...
def timeseries_rep3 (dpt_log, seaice_log, massloss_log, res_flag):
    ...
    f.close()
    dpt = dpt[skipyears*peryear:(skipyears+num_years)*peryear]
    # Plot
    clf()
    plot(time, dpt)
    xlabel('Years')
    ...
    grid(True)
    savefig('drakepsgtrans_rep3.png')
    ...
    f.close()
    seaice_area = seaice_area[skipyears*peryear:(skipyears+num_years)*peryear]
    seaice_volume = seaice_volume[skipyears*peryear:(skipyears+num_years)*peryear]
    # Plot
    clf()
    plot(time, seaice_area)
    xlabel('Years')
    ylabel(r'Total sea ice area (million km$^2$)')
    grid(True)
    savefig('seaice_area_rep3.png')
    clf()
    plot(time, seaice_volume)
    xlabel('Years')
    ylabel(r'Total sea ice volume (million km$^3$)')
    grid(True)
    savefig('seaice_volume_rep3.png')

    # Mass loss
    massloss = empty([len(names), len(dpt)])
    # Read logfile
    ...
    while index < len(names):
        ...
        for line in f:
            try:
                ...
                break
        massloss[index,:] = massloss_tmp[skipyears*peryear:(skipyears+num_years)*peryear]
        index += 1
    f.close()
    # Plot each ice shelf
    for index in range(len(names)):
        ...
        ymin = amin([ismr_low/factor, massloss_low, amin(massloss[index,:])])
        ymax = amax([ismr_high/factor, massloss_high, amax(massloss[index,:])])
        ...
        title(names[index])
        fig.savefig(fig_heads[index] + '_rep3.png')
